...
export function registerMenuGlobalHandlers(Alpine: AlpineInstance): void {
    ...
    document.addEventListener("keydown", (event) => {
        ...
        if (
            delegateToOpenMenuRoots(Alpine, event, "context-menu", (menu) =>
                Boolean(menu.panelOpen),
            )
        ) {
            return;
        }

        if (delegateToOpenMenuRoots(Alpine, event, "menubar", (menu) => isMenubarOpen(menu))) {
            return;
        }

        // Dropdown roots handle keyboard via x-on:keydown.window (submenu-aware).
        for (const element of document.querySelectorAll('[data-slot="dropdown-menu"]')) {
            const menu = Alpine.$data(element) as MenuRootComponent | undefined;
